...
|---|---|---|
| CLI tools | done | `lidza new` (with `--packs` and `--agent`), `setup`, `dev`, `build`, `check`, `gen`, `gen resource`, `pack`, `db`, `test`, `verify`, `ship`, `benchmark`, `doctor`, `api`, `snippet`, `recipe`, `mcp` |
...
| Admin pages | done | `packs/admin`: `/admin` for `ADMIN_USERS`, overview, credentials of the mail, llm and storage providers saved sealed and applied live, token usage, outbox, jobs with retry, files; themed with `admin/theme.css` and `admin/layout.html`; plain HTML, every template |
