...
does not reimplement), **decision** (needs Agim's call).
...
| Object-relational mapping | decision | design notes choose `sqlc`: SQL in `.sql` files, generated typed Go functions and structs, no ORM. Agents write correct SQL and the generated code is deterministic. A full ORM (ent, bun) is the alternative; see "Decisions" |
...
| Server-side rendering | decision | conflicts with "one binary, no Node at runtime"; see "Decisions" |
