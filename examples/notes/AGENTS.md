...
- Task recipes, step by step, in `docs/lidza-guide.md` under "Recipes", as `lidza mcp` prompts, as skills in `.claude/skills/` (Claude Code) and `.agents/skills/` (Codex, `$name`), and as Gemini commands in `.gemini/commands/lidza/` (`/lidza:name`): <!-- lidza:recipes -->`add-api-route`, `add-resource`, `add-page`, `add-pack-capability`, `add-mcp-tool`, `send-email`, `add-recipe`, `write-test`; this app's own: `scope-query-to-signed-in-user`<!-- /lidza:recipes -->. A pattern this app uses twice is a recipe: `lidza recipe add "Title"`.
