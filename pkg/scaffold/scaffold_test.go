...
func TestNewReact(t *testing.T) {
	...
	if !strings.Contains(read("CLAUDE.md"), "<!-- lidza:recipes -->`add-api-route`, `add-resource`, `scope-query-to-signed-in-user`, `add-page`, `add-pack-capability`, `add-mcp-tool`, `send-email`, `add-background-job`, `publish-live-updates`, `add-llm-feature`, `store-file`, `add-admin-pages`, `add-recipe`, `write-test`<!-- /lidza:recipes -->") {
